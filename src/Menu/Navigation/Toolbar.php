...
class Toolbar extends Menu
{
    ...
    {
        $languageDropdown = $menu->addChild('nav_language', 20)
     ...
     * Add note Menu.
     */
    private function note(ItemInterface $menu, array $options = [])
    {

        // Root Item
        $menu->addChild('nav_note', 10)
            ->setLabel(5)
            //->setLabelAttr(['class' => 'hidden'])
            ->setRoute('admin_reclamation_index')

            ->setExtra('label_translate', false)
            ->setExtra('label_icon', 'email')
          ;
    }
}
